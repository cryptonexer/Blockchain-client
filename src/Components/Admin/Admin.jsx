...
//sending activation to server
  const activated = async (_id) => {
    const response = await axios.put(`http://13.233.237.111:3002/api/activate`, {
      id: _id
    });
...
//sending deactivation to server
  const deactivated = async (_id) => {
    const response = await axios.put(`http://13.233.237.111:3002/api/deactivate`, {
      id: _id
    });
...
//deleting user
  const remove = async (_id) => {
    const res = await axios.delete(`http://13.233.237.111:3002/api/delete/${_id}`);
...
//fetching data to display in table
  useEffect(async () => {
    try {
      //active users
      const res1 = await axios.get('http://13.233.237.111:3002/api/activeUsers');
      setUsers(res1.data.data1);

      //inactive users
      const res2 = await axios.get('http://13.233.237.111:3002/api/deactiveUsers');
