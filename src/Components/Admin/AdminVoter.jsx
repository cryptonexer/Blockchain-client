...
function AdminVoter() {
  ...
  //sending deactivation to server
    const NotVerify = async (_id) => {
      const response = await axios.put(`http://13.233.237.111:3002/api/Voter/decline`, {
        id: _id
      });
  ...
  //fetching data to display in table
    useEffect(async () => {
      try {
        //Verified Voter
        const res1 = await axios.get('http://13.233.237.111:3002/api/activeVoters');
        setVoters(res1.data.data1);
  
        //Not Verified Voters
        const res2 = await axios.get('http://13.233.237.111:3002/api/deactiveVoters');
